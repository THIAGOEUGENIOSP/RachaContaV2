...
DO $$ 
BEGIN
  DROP POLICY IF EXISTS "Enable read access for authenticated users" ON carnivals;
  DROP POLICY IF EXISTS "Enable write access for authenticated users" ON carnivals;
  DROP POLICY IF EXISTS "Enable read access for carnival participants" ON carnival_participants;
  DROP POLICY IF EXISTS "Enable write access for carnival participants" ON carnival_participants;
EXCEPTION
  WHEN undefined_object THEN NULL;
END $$;

-- Add RLS policies
CREATE POLICY "authenticated_read_carnivals"
  ON carnivals FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "authenticated_write_carnivals"
  ON carnivals FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "authenticated_update_carnivals"
  ON carnivals FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "authenticated_delete_carnivals"
  ON carnivals FOR DELETE
  TO authenticated
  USING (true);

CREATE POLICY "authenticated_read_carnival_participants"
  ON carnival_participants FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "authenticated_write_carnival_participants"
  ON carnival_participants FOR INSERT
  TO authenticated
  WITH CHECK (true);

CREATE POLICY "authenticated_update_carnival_participants"
  ON carnival_participants FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

CREATE POLICY "authenticated_delete_carnival_participants"
